...
namespace Koshi_giu.Boundary_solver
{
    ...
    class Choot : Step_Solver
    {
        ...
        public void Form_Cauchy( Boundary task, double x, Function [ ] funcs_diffs, Cauchy_Solver solver )
        {
            Function [ ] right_funcs = Form_right_funcs( task.Right_func, funcs_diffs );
        ...
        private Value [ ] Form_start_values( Value task_start_value , Func<double [ ], double> task_start_boundary_func)
        {
            ...
            if ( task_start_boundary_func( new double [ ] { 0, 1 } ) != 0 )
            {
                 start_value [ 0 ].U [ 2 ] = 1;
        ...
        public override double [ ] Solve( Boundary task, double x, double So, Function [ ] funcs_diff, Cauchy_Solver cauchy_solver )
        {
            double h = 0.01;
            double eps = 0.00001;
            ...
            double S_next = S;
            double [ ] cauchy_res = new double[4];
            ...
            do
            {
                ...
                if ( task.Boundary_funcs [ 0 ]( new double [ ] { 0, 1 } ) != 0 )
                {
                ...
                S_next = S - ( task.Boundary_funcs [ 1 ]( new double [ ] { cauchy_res [ 0 ], cauchy_res [ 1 ] } ) - task.Values [ 1 ].U [ 0 ] )
                             / task.Boundary_funcs [ 1 ]( new double [ ] { cauchy_res [ 2 ], cauchy_res [ 3 ] } );
